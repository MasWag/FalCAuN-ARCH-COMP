#!/bin/sh -u
#****h* transmission/run_falcaun_AT5
# NAME
#  run_falcaun_AT5.sh
# DESCRIPTION
#  Script to falsify the AT5 formulas by FalCAuN
# AUTHOR
#  Masaki Waga
# HISTORY
...
#****if* run_falcaun_AT5/configuration
# DESCRIPTION
#  Removes the temporary files at exit
# SOURCE
#
atexit() {
...
#****d* run_falcaun_AT5/configuration
# DESCRIPTION
...
#****d* run_falcaun_AT5/input_mapper
# DESCRIPTION
...
#****d* run_falcaun_AT5/output_mapper
# DESCRIPTION
...
#****d* run_falcaun_AT5/signal_definition
# DESCRIPTION
...
#****d* run_falcaun_AT5/stl_file
# DESCRIPTION
#  Define the STL formulas to be falsified
#
#  We remark that the formulas are NOT the same as the ones in the original paper due the discrete-time semantics of FalCAuN.
# NOTES
#  In particular, we note the following:
#  - Since the signal step is 2.0, the time bounds are divided by two, i.e., 30 -> 15 and 2.5 -> 1.
#  - The next operator is implemented by the next operator in the discrete-time semantics rather than an eventually operator with a short interval.
#
# SOURCE
#
alw_[0, 15] (($gear != 1 && X ($gear == 1)) -> X (alw_[0, 1] ($gear == 1)))
alw_[0, 15] (($gear != 2 && X ($gear == 2)) -> X (alw_[0, 1] ($gear == 2)))
alw_[0, 15] (($gear != 3 && X ($gear == 3)) -> X (alw_[0, 1] ($gear == 3)))
alw_[0, 15] (($gear != 4 && X ($gear == 4)) -> X (alw_[0, 1] ($gear == 4)))
#******
EOF
