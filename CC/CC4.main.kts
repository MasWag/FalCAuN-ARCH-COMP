 ...
 *   CC4.main.kts
 *  DESCRIPTION
 *   Script to falsify the chasing cars benchmark against the CC4 formula by FalCAuN
 *  AUTHOR
 *   Masaki Waga
 *  HISTORY
...
logger.info("This is the script to falsify the chasing car benchmark against the CC4 formula by FalCAuN")
...
val y5y4Values = listOf(8.0, null)
...
val mapperString = listOf("previous_min($y5 - $y4)").joinToString("\n")
...
val stlList =
    listOf(
        "alw_[0,${(65 / signalStep).toInt()}] ev_[0,${(30 / signalStep).toInt()}] alw_[0,${(5 / signalStep).toInt()}] $diffy5y4 > 8",
...
SimulinkSUL(initScript, paramNames, signalStep, simulinkSimulationStep).use { sul ->
    ...
    for (i in 0 until experimentSize) {
        ...
        // Run the experiment
        var result = runExperiment(verifier, simulinkSimulationStep, "CC", "CC4")
        results.add(result)
    }
    FileOutputStream("result-CC4.csv").apply { writeCsv(results) }
    logger.info("The results are written to result-CC4.csv")
}
