#!/bin/sh -u
#****h* transmission/run_falcaun_CC1
# NAME
#  run_falcaun_CC1.sh
# DESCRIPTION
#  Script to falsify the CC1 formula by FalCAuN
# AUTHOR
#  Masaki Waga
# HISTORY
...
#****if* run_falcaun_CC1/configuration
# DESCRIPTION
#
# SOURCE
#
...
#****d* run_falcaun_CC1/configuration
# DESCRIPTION
...
input_mapper=$(mktemp /tmp/CC1.XXXXXX.imap.tsv)
cat <<EOF | sed 's/#.*$//;/^$/d;' > "$input_mapper"
#****d* run_falcaun_CC1/input_mapper
# DESCRIPTION
...
signal_mapper=$(mktemp /tmp/CC1.XXXXXX.sigmap)
cat <<EOF | sed 's/#.*$//;/^$/d;' > "$signal_mapper"
#****d* run_falcaun_CC1/signal_mapper
# DESCRIPTION
...
output_mapper=$(mktemp /tmp/CC1.XXXXXX.omap.tsv)
cat <<EOF | sed 's/#.*$//;/^$/d;' > "$output_mapper"
#****d* run_falcaun_CC1/output_mapper
# DESCRIPTION
...
stl_file=$(mktemp /tmp/CC1.XXXXXX.stl)
cat <<EOF | sed 's/#.*$//;/^$/d;' > "$stl_file"
#****d* run_falcaun_CC1/stl_file
# DESCRIPTION
