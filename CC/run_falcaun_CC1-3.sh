#!/bin/sh -u
#****h* transmission/run_falcaun_CC1-3
# NAME
#  run_falcaun_CC1-3.sh
# DESCRIPTION
#  Script to falsify the CC1, CC2, and CC3 formulas by FalCAuN
# AUTHOR
#  Masaki Waga
# HISTORY
...
#****if* run_falcaun_CC1-3/configuration
# DESCRIPTION
#
# SOURCE
#
...
#****d* run_falcaun_CC1-3/configuration
# DESCRIPTION
...
#****d* run_falcaun_CC1-3/input_mapper
# DESCRIPTION
...
signal_mapper=$(mktemp /tmp/CC1-3.XXXXXX.sigmap)
cat <<EOF | sed 's/#.*$//;/^$/d;' > "$signal_mapper"
#****d* run_falcaun_CC1-3/signal_mapper
# DESCRIPTION
...
output_mapper=$(mktemp /tmp/CC1-3.XXXXXX.omap.tsv)
cat <<EOF | sed 's/#.*$//;/^$/d;' > "$output_mapper"
#****d* run_falcaun_CC1-3/output_mapper
# DESCRIPTION
...
stl_file=$(mktemp /tmp/CC1-3.XXXXXX.stl)
cat <<EOF | sed 's/#.*$//;/^$/d;' > "$stl_file"
#****d* run_falcaun_CC1-3/stl_file
# DESCRIPTION
