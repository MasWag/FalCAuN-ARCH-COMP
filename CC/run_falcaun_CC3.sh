#!/bin/sh -u
#****h* transmission/run_falcaun_CC3
# NAME
#  run_falcaun_CC3.sh
# DESCRIPTION
#  Script to falsify the CC3 formula by FalCAuN
# AUTHOR
#  Masaki Waga
# HISTORY
...
#****if* run_falcaun_CC3/configuration
# DESCRIPTION
#
# SOURCE
#
...
#****d* run_falcaun_CC3/configuration
# DESCRIPTION
...
input_mapper=$(mktemp /tmp/CC3.XXXXXX.imap.tsv)
cat <<EOF | sed 's/#.*$//;/^$/d;' > "$input_mapper"
#****d* run_falcaun_CC3/input_mapper
# DESCRIPTION
...
signal_mapper=$(mktemp /tmp/CC3.XXXXXX.sigmap)
cat <<EOF | sed 's/#.*$//;/^$/d;' > "$signal_mapper"
#****d* run_falcaun_CC3/signal_mapper
# DESCRIPTION
...
output_mapper=$(mktemp /tmp/CC3.XXXXXX.omap.tsv)
cat <<EOF | sed 's/#.*$//;/^$/d;' > "$output_mapper"
#****d* run_falcaun_CC3/output_mapper
# DESCRIPTION
...
stl_file=$(mktemp /tmp/CC3.XXXXXX.stl)
cat <<EOF | sed 's/#.*$//;/^$/d;' > "$stl_file"
#****d* run_falcaun_CC3/stl_file
# DESCRIPTION
